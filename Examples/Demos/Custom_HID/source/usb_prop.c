...
void CustomHID_init(void)
{
  ...
  _SetCNTR(wInterrupt_Mask); /* set interrupts mask */			//设置中断

  bDeviceState = UNCONNECTED;		//USB状态未连接
}
...
void CustomHID_Reset(void)
{
  ...
  pInformation->Current_Configuration = 0;		//配置为0
  pInformation->Current_Interface = 0;/*the default Interface*/
  
  /* Current Feature initialization */	//DESC.C
  pInformation->Current_Feature = CustomHID_ConfigDescriptor[7];
  
  SetBTABLE(BTABLE_ADDRESS);		//设置缓冲区表地址

  /* Initialize Endpoint 0 */
  SetEPType(ENDP0, EP_CONTROL);	//初始化端点--控制接口
  SetEPTxStatus(ENDP0, EP_TX_STALL);	//端点名--发送
  SetEPRxAddr(ENDP0, ENDP0_RXADDR);		//端点接收地址
  SetEPTxAddr(ENDP0, ENDP0_TXADDR);		//端点发送地址
  Clear_Status_Out(ENDP0);						//清除端点状态
  SetEPRxCount(ENDP0, Device_Property.MaxPacketSize);	//设置最大数据包大小
  SetEPRxValid(ENDP0);								//使能端点0

  /* Initialize Endpoint 1 */			//设置端点1
