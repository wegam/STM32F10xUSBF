...
/* Exported macro ------------------------------------------------------------*/
/* Exported functions ------------------------------------------------------- */
void FLASH_SetLatency(u32 FLASH_Latency);										//设置代码延时值
void FLASH_HalfCycleAccessCmd(u32 FLASH_HalfCycleAccess);		//使能或者失能FLASH半周期访问
void FLASH_PrefetchBufferCmd(u32 FLASH_PrefetchBuffer);			//使能或者失能预取指缓存

#ifdef _FLASH_PROG
void FLASH_Unlock(void);		//解锁FLASH编写擦除控制器											
void FLASH_Lock(void);			//锁定FLASH编写擦除控制器
FLASH_Status FLASH_ErasePage(u32 Page_Address);		//擦除一个FLASH页面
FLASH_Status FLASH_EraseAllPages(void);						//擦除全部FLASH页面
FLASH_Status FLASH_EraseOptionBytes(void);				//擦除FLASH选择字节
FLASH_Status FLASH_ProgramWord(u32 Address, u32 Data);					//在指定地址编写一个字
FLASH_Status FLASH_ProgramHalfWord(u32 Address, u16 Data);			//在指定地址编写半字
FLASH_Status FLASH_ProgramOptionByteData(u32 Address, u8 Data);	//在指定FLASH选择字节地址编写半字
FLASH_Status FLASH_EnableWriteProtection(u32 FLASH_Pages);			//对选定的页面写保护
FLASH_Status FLASH_ReadOutProtection(FunctionalState NewState);	//使能或者失能读出保护
FLASH_Status FLASH_UserOptionByteConfig(u16 OB_IWDG, u16 OB_STOP, u16 OB_STDBY);	//编写FLASH用户选择字节：IWDG_SW /RST_STOP /RST_STDBY
u32 FLASH_GetUserOptionByte(void);									//返回FLASH用户选择字节的值
u32 FLASH_GetWriteProtectionOptionByte(void);				//返回FLASH写保护选择字节的值
FlagStatus FLASH_GetReadOutProtectionStatus(void);	//检查FLASH读出保护设置与否	
FlagStatus FLASH_GetPrefetchBufferStatus(void);			//检查FLASH预取指缓存设置与否
void FLASH_ITConfig(u16 FLASH_IT, FunctionalState NewState);	//使能或者失能指定FLASH中断
FlagStatus FLASH_GetFlagStatus(u16 FLASH_FLAG);			//检查指定的FLASH标志位设置与否
void FLASH_ClearFlag(u16 FLASH_FLAG);								//清除FLASH待处理标志位
FLASH_Status FLASH_GetStatus(void);									//返回FLASH状态
FLASH_Status FLASH_WaitForLastOperation(u32 Timeout);	//等待某一个Flash操作完成，或者发生TIMEOUT
#endif
