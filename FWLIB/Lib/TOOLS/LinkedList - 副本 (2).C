...
//		if(LISTx->HeadNode->NextNode	==	NULL)			//会造成溢出
//		{
//			DeleteNode((LISTx->LastNode));										//删除结点
//			LISTx->LastNode	=	NULL;
//		}		
...
//		LISTx->DataAddr		=	NULL;						//数据地址为空
	...
	else		
	{
		...
		LINK_NODE *TempNode = LISTx->HeadNode; 														//临时数据
		
		ListLength	=	GetListLength(LISTx->HeadNode)+1;											//获取链表长度
		
		for(i=0;i<=ListLength;i++)
		{
			if(memcmp(TempNode->DataAddr,DataAddr,DataLength))							//比较内存数据相等
			{
				DeleteNode(TempNode);																					//删除本结点
				return	1;		//
			}
			else
			{
				...
				{
					memset(DataAddr,0x9F,DataLength);
					return 0;					
				}
			}
		}
