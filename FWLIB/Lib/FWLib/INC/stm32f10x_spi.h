...
typedef struct
{
  u16 SPI_Direction;					//设置方向				（2线全双工、2线只接收、一线发送、一线接收）
  u16 SPI_Mode;								//模式         	（从或主设备）
  u16 SPI_DataSize;						//宽度         	（8或16位）
  u16 SPI_CPOL;								//时钟极性     	（低或高）
  u16 SPI_CPHA;					 			//时钟相位     	（第一个或第二个跳变沿）
  u16 SPI_NSS;								//片选方式     	（硬件或软件方式）
  u16 SPI_BaudRatePrescaler;	//波特率预分频 	（从2---256分频）
  u16 SPI_FirstBit;						//最先发送的位 	（最低位，还是最高位在先）
  u16 SPI_CRCPolynomial;			//设置crc多项式 （数字）如7
}SPI_InitTypeDef;
...
void SPI_Init(SPI_TypeDef* SPIx, SPI_InitTypeDef* SPI_InitStruct);											//根据SPI_InitStruct中指定的参数初始化外设SPIx寄存器
void I2S_Init(SPI_TypeDef* SPIx, I2S_InitTypeDef* I2S_InitStruct);
void SPI_StructInit(SPI_InitTypeDef* SPI_InitStruct);																		//把SPI_InitStruct中的每一个参数按缺省值填入
void I2S_StructInit(I2S_InitTypeDef* I2S_InitStruct);
void SPI_Cmd(SPI_TypeDef* SPIx, FunctionalState NewState);															//使能或者失能SPI外设
void I2S_Cmd(SPI_TypeDef* SPIx, FunctionalState NewState);
void SPI_I2S_ITConfig(SPI_TypeDef* SPIx, u8 SPI_I2S_IT, FunctionalState NewState);			//使能或者失能指定的SPI中断
void SPI_I2S_DMACmd(SPI_TypeDef* SPIx, u16 SPI_I2S_DMAReq, FunctionalState NewState);		//使能或者失能指定SPI的DMA请求
void SPI_I2S_SendData(SPI_TypeDef* SPIx, u16 Data);																			//通过外设SPIx发送一个数据
u16 SPI_I2S_ReceiveData(SPI_TypeDef* SPIx);																							//返回通过SPIx最近接收的数据
void SPI_NSSInternalSoftwareConfig(SPI_TypeDef* SPIx, u16 SPI_NSSInternalSoft);					//为选定的SPI软件配置内部NSS管脚
void SPI_SSOutputCmd(SPI_TypeDef* SPIx, FunctionalState NewState);											//使能或者失能指定的SPI SS输出
void SPI_DataSizeConfig(SPI_TypeDef* SPIx, u16 SPI_DataSize);														//设置选定的SPI数据大小-----SPI_DataSize设置8位或者16位数据帧结构
void SPI_TransmitCRC(SPI_TypeDef* SPIx);																								//使能或者失能指定SPI的CRC传输
void SPI_CalculateCRC(SPI_TypeDef* SPIx, FunctionalState NewState);											//使能或者失能指定SPI的传输字CRC值计算
u16 SPI_GetCRC(SPI_TypeDef* SPIx, u8 SPI_CRC);																					//返回指定SPI的CRC值
u16 SPI_GetCRCPolynomial(SPI_TypeDef* SPIx);																						//返回指定SPI的CRC多项式寄存器值
void SPI_BiDirectionalLineConfig(SPI_TypeDef* SPIx, u16 SPI_Direction);									//选择指定SPI在双向模式下的数据传输方向
FlagStatus SPI_I2S_GetFlagStatus(SPI_TypeDef* SPIx, u16 SPI_I2S_FLAG);									//检查指定的SPI标志位设置与否
void SPI_I2S_ClearFlag(SPI_TypeDef* SPIx, u16 SPI_I2S_FLAG);														//清除SPIx的待处理标志位
ITStatus SPI_I2S_GetITStatus(SPI_TypeDef* SPIx, u8 SPI_I2S_IT);													//检查指定的SPI中断发生与否
void SPI_I2S_ClearITPendingBit(SPI_TypeDef* SPIx, u8 SPI_I2S_IT);												//清除SPIx的中断待处理位
