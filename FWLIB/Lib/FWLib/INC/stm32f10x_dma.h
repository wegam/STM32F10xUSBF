...
*******************************************************************************/
/*—————————————————《DMA控制器（DMA）》———————————————/
*	DMA控制器提供7个数据通道的访问。由于外设实现了向存储器的映射，因此数据对来自或者发向外设
*	的数据传输，也可以像内存之间的数据传输一样管理。
——————————————————————————————————————————*/
...
typedef struct
{
  ...
  u32 DMA_DIR;										//DMA_DIR规定了外设是作为数据传输的目的地还是来源。<DMA data transfer direction>给出了该参数的取值范围。
  u32 DMA_BufferSize;							//DMA_BufferSize用以定义指定DMA通道的DMA缓存的大小，单位为数据单位。根据传输方向，数据单位等于结构中参数DMA_PeripheralDataSize或者参数DMA_MemoryDataSize的值。
  u32 DMA_PeripheralInc;					//DMA_PeripheralInc用来设定外设地址寄存器递增与否。<DMA peripheral incremented mode>给出了该参数的取值范围。
  u32 DMA_MemoryInc;							//DMA_MemoryInc用来设定内存地址寄存器递增与否。<DMA memory incremented mode>给出了该参数的取值范围。
  u32 DMA_PeripheralDataSize;			//DMA_PeripheralDataSize设定了外设数据宽度。<DMA peripheral data size>给出了该参数的取值范围。
  u32 DMA_MemoryDataSize;					//DMA_MemoryDataSize设定了外设数据宽度。<DMA memory data size>给出了该参数的取值范围。
  u32 DMA_Mode;										//DMA_Mode设置了DMA的工作模式，<DMA circular/normal mode>给出了该参数可取的值
  u32 DMA_Priority;								//DMA_Priority设定DMA通道x的软件优先级。<DMA priority level>给出了该参数可取的值。
  u32 DMA_M2M;										//DMA_M2M使能DMA通道的内存到内存传输。<DMA memory to memory>给出了该参数可取的值。
}DMA_InitTypeDef;
...
/* For DMA1 */
#define DMA1_IT_GL1                        ((u32)0x00000001)	//通道1全局标志位
#define DMA1_IT_TC1                        ((u32)0x00000002)	//通道1传输完成标志位
#define DMA1_IT_HT1                        ((u32)0x00000004)	//通道1传输过半标志位
#define DMA1_IT_TE1                        ((u32)0x00000008)	//通道1传输错误标志位
...
void DMA_ITConfig(DMA_Channel_TypeDef* DMAy_Channelx, u32 DMA_IT, FunctionalState NewState);	//使能或者失能指定的通道x中断
u16 DMA_GetCurrDataCounter(DMA_Channel_TypeDef* DMAy_Channelx);																//返回当前DMA通道x剩余的待传输数据数目
